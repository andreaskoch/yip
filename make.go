...
func gitVersion() string {
	...
	latestTag := ""
	if gitLatestTagCommandOutput, _ := gitLatestTagCommand.Output(); len(gitLatestTagCommandOutput) > 0 {
		latestTag = strings.TrimSpace(string(gitLatestTagCommandOutput))
	}

	// use the current branch name instead of the tag if there is no tag
	if latestTag == "" {
		gitBranchNameCommand := exec.Command("git", "rev-parse", "--abbrev-ref", "HEAD")
		gitBranchNameCommand.Dir = goPath

		gitBranchNameCommandOutput, _ := gitBranchNameCommand.Output()
		latestTag = strings.TrimSpace(string(gitBranchNameCommandOutput))
	}
...
// Get the build version flag for the go linker
